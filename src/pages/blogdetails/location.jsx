...
const LocationSection = ({ dealData }) => {
  const mapStyles = {
    height: "400px",
    width: "100%",
    borderRadius: "8px",
  ...
  const defaultCenter = {
    lat: parseFloat(dealData.lat),
    lng: parseFloat(dealData.lang),
  };
  ...
  return (
    <div>
      ...
      <p className="text-gray-800 mb-2">{dealData.address}</p>
      <p className="text-gray-600 mb-4">{dealData.type}</p>
      ...
      <div className="relative w-full h-[400px] bg-gray-100 rounded-lg">
        {!mapError ? (
          ...
          >
            ...
            >
              <Marker position={defaultCenter} title={dealData.name} />
            </GoogleMap>
          </LoadScript>
        ) : (
